...
public class PedidoService {
    ...
    public PedidoDTO criar(PedidoCriacaoDTO dto) {
        ...
        for (ItemPedidoCriacaoDTO itemDTO : dto.getItens()) {
            ...
            BigDecimal desconto = primeiroPedido ? descontoService.calcularDesconto(p.getPreco()) : BigDecimal.ZERO;
    ...
    private PedidoDTO toDto(Pedido p) {
        ...
        List<ItemPedidoDTO> itensDto = p.getItens().stream().map(item -> {
            ...
            idto.setSubtotal(item.getValorVenda().multiply(BigDecimal.valueOf(item.getQuantidade()))
